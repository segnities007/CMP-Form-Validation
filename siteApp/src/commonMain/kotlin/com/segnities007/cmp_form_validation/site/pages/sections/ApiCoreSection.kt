...
fun ApiCoreSection() {
    ApiSection(
        ...
        rows = {
            ApiRow(
                "Rule<T>",
                stringResource(Res.string.api_rule_desc),
                "fun interface Rule<T> { fun validate(value: T): ValidationError? }",
            )
            ApiRow(
                "Validator<T>",
                stringResource(Res.string.api_validator_desc),
                "class Validator<T>(rules: List<Rule<T>>, strategy: ValidationStrategy)",
            )
            ApiRow(
                "ValidationResult",
                stringResource(Res.string.api_result_desc),
                "data class ValidationResult(errors: ImmutableList<ValidationError>)",
            )
            ApiRow(
                "ValidationError",
                stringResource(Res.string.api_error_desc),
                "data class ValidationError(code: String, defaultMessage: String, path: String?, meta: ImmutableMap<String, String>)",
            )
